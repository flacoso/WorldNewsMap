...
export default function WorldMap({news,onSelect}:{news:MapNews[];onSelect:(id:string)=>void}){
 ...
 useEffect(()=>{
  ...
  const load=async()=>{const L=await import('leaflet');if(cancelled||!ref.current)return;
   ...
   render();const resize=()=>map.invalidateSize(false);const ro=new ResizeObserver(resize);ro.observe(ref.current);window.addEventListener('resize',resize);requestAnimationFrame(resize);setTimeout(resize,250);setTimeout(resize,1000);
 ...
 useEffect(()=>{newsRef.current=news;const LGroup=markersRef.current;if(!LGroup)return;LGroup.clearLayers();},[news]);
