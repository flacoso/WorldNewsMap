...
export default function WorldMap({news,onSelect}:{news:MapNews[];onSelect:(id:string)=>void}){
 ...
 useEffect(()=>{
  ...
  const map=new maplibregl.Map({container:ref.current,style:STYLE,center:[0,20],zoom:1.25,minZoom:1,maxZoom:8,attributionControl:{compact:true}});
  mapRef.current=map;map.addControl(new maplibregl.NavigationControl(),'top-right');
  const render=()=>{
   const data=geo(newsRef.current);
  ...
  const resize=()=>map.resize();const ro=new ResizeObserver(resize);ro.observe(ref.current);window.addEventListener('resize',resize);requestAnimationFrame(resize);setTimeout(resize,250);setTimeout(resize,1000);
  return()=>{ro.disconnect();window.removeEventListener('resize',resize);map.remove();mapRef.current=null};
 },[]);
 useEffect(()=>{const map=mapRef.current;if(!map||!map.isStyleLoaded())return;const src=map.getSource('news') as maplibregl.GeoJSONSource|undefined;if(src)src.setData(geo(news));},[news]);
